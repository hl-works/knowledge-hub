...
5. **Galerie** — agrège **toutes les photos/vidéos des escales** (Sheet + repo, source unique, pas de double saisie), **rangées par escale** (`escaleHead()` : ville + pays + dates + tampon pays), escale la plus récente d'abord, dernier média ajouté en tête. Grille graphique, lightbox (vidéos jouables). Le **Parcours** montre la même galerie légendée par escale, sous le récit/lieu.
...
- **Groupage** : les listes Hôtels/Parcours sont sectionnées par pays via `paysHead()` ; la **Galerie** est sectionnée par **escale** via `escaleHead()` (ville en titre, pays + dates en sous-titre). Les deux vivent dans `src/lib/paysMeta.ts` (source unique palette/codes — miroir Python dans `scripts/build_atlas_static.py`). Chaque en-tête contient un **`.stamp-slot`** avec le tampon AMHE « eki » du pays.
